    ...
    <div class="container-fluid">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card card-maroon">
                ...
                <a href="<?php echo site_url('sppd_luar/add');?>" 
                class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Data Kegiatan</a>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                <table id="example" class="table table-responsive table-hover" style="width:100%">
                    ...
                    <tr>
                        ...
                        <td><center><?php if(getjumpegawai($row->id) >= 1 && getstatus($row->id) == 1){
                            echo "<btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Input Pegawai'><i class='fas fa-user'></i></btn> 
                            ...
                            echo "<btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Input Pegawai'><i class='fas fa-user'></i></btn> 
                            <btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Cetak SPT'><i class='fas fa-file-alt'></i></i></btn>
                            <btn class='btn bg-dark btn-sm' data-toggle='tooltip' title='Belum Cetak Visum'><i class='fas fa-print'></i></btn>
                            <btn class='btn bg-dark btn-sm' data-toggle='tooltip' title='Belum Input LHPD'><i class='fas fa-clipboard-list'></i></btn> 
                            <btn class='btn bg-dark btn-sm' data-toggle='tooltip' title='Belum Divalidasi'><i class='fas fa-clipboard-check'></i></btn>";
                            ...
                            echo "<btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Input Pegawai'><i class='fas fa-user'></i></btn>
                            <btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Cetak SPT'><i class='fas fa-file-alt'></i></i></btn>
                            <btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Cetak Visum'><i class='fas fa-print'></i></btn>
                            <btn class='btn bg-dark btn-sm' data-toggle='tooltip' title='Belum Input LHPD'><i class='fas fa-clipboard-list'></i></btn>
                            <btn class='btn bg-dark btn-sm' data-toggle='tooltip' title='Belum Divalidasi'><i class='fas fa-clipboard-check'></i></btn>";
                        }elseif(getjumpegawai($row->id) >= 1 && getstatus($row->id) == 4){
                            echo "<btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Input SPT'><i class='fas fa-user'></i></btn> 
                            <btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Cetak SPT'><i class='fas fa-file-alt'></i></i></btn>  
                            <btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Cetak Visum'><i class='fas fa-print'></i></btn> 
                            <btn class='btn bg-maroon btn-sm' data-toggle='tooltip' title='Sudah Input LHPD'><i class='fas fa-clipboard-list'></i></btn> 
                            <btn class='btn bg-dark btn-sm' data-toggle='tooltip' title='Belum Divalidasi'><i class='fas fa-clipboard-check'></i></btn>";
                        ...
                        <td>
                        <a href="<?php echo base_url('kwitansi_luar/kwitansi/'.$row->id);?>" class="btn bg-maroon"><i class="fas fa-shopping-cart"></i> RINCIAN SPPD
                        </a>
                        </td>
                        <td><center>
                            <a href="<?php echo site_url('sppd_luar/hapus/'.$row->id); ?>" 
                            class="btn btn-danger btn-sm tombolhapus"><i class="fas fa-trash"></i> HAPUS</a>
                            <a href="<?php echo base_url('sppd_luar/edit/'.$row->id);?>" 
                            class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> EDIT</a>
                            <br>
                            <button data-toggle="modal" data-target="#modal_qr_<?=$row->id;?>" 
                            class="btn bg-black btn-sm"> <i class="fas fa-qrcode"></i> QRCODE</button> 
                            <?php if(getjumpegawai($row->id) >= 1 && getstatus($row->id) >= 3){
                                echo '<a href="'.base_url('sppd_luar/lhpd/'.$row->id).'" 
